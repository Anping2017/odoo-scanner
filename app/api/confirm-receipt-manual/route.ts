// /app/api/confirm-receipt-manual/route.ts
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. 如果指定了特定产品，先设置这些产品的 quantity
    if (product_ids && Array.isArray(product_ids) && product_ids.length > 0) {
      console.log('设置指定产品的 quantity:', product_ids);
      ...
      for (const move of movesToUpdate) {
        if (move.product_qty > 0) {
          ...
          if (tracking !== 'none') {
            // 需要批次/序列号跟踪，检查是否有手动输入的Lot/Serial Number
            const manualLotSerial = lot_serial_numbers?.[move.id];
            
            if (manualLotSerial) {
              console.log(`产品 ${move.product_id[1]} 需要 ${tracking} 跟踪，使用手动输入的Lot/Serial Number: ${manualLotSerial}`);
              ...
              try {
                const moveLineData = await rpc(base, '/web/dataset/call_kw', {
                  ...
                  args: [{
                    ...
                    lot_name: manualLotSerial, // 使用手动输入的Lot/Serial Number
                    lot_id: false,
              ...
              console.log(`产品 ${move.product_id[1]} 需要 ${tracking} 跟踪但没有提供Lot/Serial Number，跳过`);
              // 如果没有提供Lot/Serial Number，跳过这个产品
              continue;
            }
          } else {
            // 不需要跟踪，直接设置数量
    ...
    return NextResponse.json({ 
      success: true, 
      message: '一键入库完成！手动输入的批次/序列号已处理，收货单已验证',
